...
/**
 * The "y" option for dynamic selectors.
 *
 * @package Celestriode\Mattock\Parsers\Java\Selector\Options
 */
class Y extends AbstractOption
{
    /**
    ...
    {
        $y = $parser->getReader()->readDouble();

        $parser->getBuilder()->getSelector()->getOrigin()->setY($y);
    ...
    /**
     * Can only specify a Y coordinate if it wasn't already specified.
     *
    ...
    {
        return $builder->getSelector()->getOrigin()->getY() === null;
    }
}