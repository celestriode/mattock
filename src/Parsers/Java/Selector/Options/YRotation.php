...
/**
 * The "y_rotation" option for dynamic selectors.
 *
 * @package Celestriode\Mattock\Parsers\Java\Selector\Options
 */
class YRotation extends AbstractOption
{
    /**
    ...
    public static function handle(EntitySelectorParser $parser): AbstractOption
    {
        ...
        $parser->getBuilder()->getSelector()->getYRotation()->setFromBounds($bounds);

        return new self($bounds);
    }

    /**
     * Can only specify a Y rotation if it hasn't already been specified.
     *
    ...
    {
        return $builder->getSelector()->getYRotation()->isAny();
    }
}